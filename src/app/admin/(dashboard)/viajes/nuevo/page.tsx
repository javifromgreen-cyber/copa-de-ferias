...
export default async function NewTripPage() {
  const [maxNumber, feeConfig] = await Promise.all([
    prisma.trip.aggregate({ _max: { number: true } }),
  ...
  return (
    <div>
      ...
      <TripForm initial={{ ...BLANK_TRIP_FORM, number: (maxNumber._max.number ?? 0) + 1 }} globalFeeConfig={feeConfig} />
    </div>
  );
}
